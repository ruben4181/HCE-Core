...
DELIMITER //
CREATE PROCEDURE deleteExamen_Fisico (IN ID BIGINT)
BEGIN
	IF (SELECT EXISTS (SELECT idExamen FROM Examen_Fisico WHERE idExamen = ID)) THEN
		START TRANSACTION;
...
DELIMITER //
CREATE PROCEDURE deleteHabito (IN ID BIGINT)
BEGIN
	IF (SELECT EXISTS (SELECT idHabito FROM Habitos WHERE idHabito = ID) ) THEN
		START TRANSACTION;
...
DELIMITER //
CREATE PROCEDURE deleteExamenSegmentario (IN ID BIGINT)
BEGIN
	IF (SELECT EXISTS (SELECT idExamen FROM ExamenSegmentario WHERE idExamen = ID) ) THEN
		START TRANSACTION;
...
DELIMITER //
CREATE PROCEDURE updateCitas_Medicas(IN ID BIGINT, IN FECHA TIMESTAMP,IN MOTIVO VARCHAR(200), IN EPSAGENDA BIGINT,IN MEDICOS_IDMEDICOS BIGINT,
								 IN EXAMEN_FISICO_IDEXAMEN BIGINT, IN ID_HAB BIGINT,IN ID_EX_SEG BIGINT, IN HISTORIA_CLINICA_IDHISTORIA BIGINT)
BEGIN
	IF (SELECT EXISTS (SELECT idConsulta FROM Citas_Medicas WHERE idConsulta = ID)) THEN
		START TRANSACTION;
...
DELIMITER //
CREATE PROCEDURE deleteCitas_Medicas (IN ID BIGINT)
BEGIN
	IF (SELECT  EXISTS (SELECT idConsulta FROM Citas_Medicas WHERE idConsulta = ID))  THEN
		START TRANSACTION;
